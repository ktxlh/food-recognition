...
# def get_deeplabv3(dataset='pascal_voc', backbone='resnet50', pretrained=False, root='~/.torch/models', pretrained_base=True, **kwargs):
def get_deeplabv3(dataset='pascal_voc', backbone='resnet50', pretrained=False, root='~/.torch/models', pretrained_base=True, **kwargs):
    acronyms = {
    ...
    if pretrained:
        ...
        #     print("!!!")
        #     model.load_state_dict(torch.load('https://github.com/LikeLy-Journey/SegmenTron/releases/download/v0.1.0/resnet50-25c4b509.pth', map_location=device))
        # else:
        #     print("@@@")
        model.load_state_dict(torch.load(get_model_file('deeplabv3_%s_%s' % (backbone, acronyms[dataset]), root=root), map_location=device))
    return model
...
def get_deeplabv3_resnet50_foodrecognition(**kwargs):
    return get_deeplabv3('foodrecognition', 'resnet50', **kwargs)


if __name__ == '__main__':
    # model = get_deeplabv3_resnet50_voc()
    model = get_deeplabv3_resnet50_foodrecognition()
